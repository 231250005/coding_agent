...
PlanExecuteStrategy（顶层外壳，恒定）
   ...
   └─ ③ 最终汇报：全部步骤完成后汇总输出（评审不作为固定步骤，
           由模型在子任务执行中按需自主调用 code_review 工具）
```

- **规划 JSON 协议**：`{goal, steps: [{id, task, mode, output}]}`，mode 由模型规划时指定
- **子任务级上下文隔离**：每个子任务独立运行自己的消息历史，完成后只保留结果摘要进全局上下文——长任务不会上下文爆炸
- **评审工具化（关键设计）**：`code_review` 与文件/执行类工具同级注册在工具注册表中，由模型在子任务执行过程中**按需自主调用**（实现完成、测试通过后自查），**不是固定的流程收尾步骤**；不设独立"反思框架"——执行性验证（跑测试看结果）已被 ReAct 循环内化，主观评审由工具按需提供
...
| **代码质量** | `code_review` | 评审者视角检查文件/git diff，输出问题清单与修复建议；模型在子任务执行中按需自主调用，是"测试通过≠任务完成"的兜底 |
...
**P1 高分加分**：Web 流式可视化；子任务级上下文隔离 + token 估算/摘要压缩；文件树 + diff 视图；计划展示；评审工具（code_review，模型按需调用）；LLM 调用预算护栏；停止按钮；token 统计；会话持久化
...
- 为什么评审工具化且按需调用而非固定收尾步骤 → 执行性验证已被 ReAct 内化；评审时机由模型自主决定（实现完成/测试通过后自查），框架只提供能力不规定时机，保持工作流灵活
