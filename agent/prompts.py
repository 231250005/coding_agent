...
4. 验证：每次写完或修改代码后，按以下顺序进行。验收标准是「代码能正常运行」，
   不是「代码完美」——评审/测试各最多 2 轮，达到轮次后必须继续下一步：
   a. 评审：调用 code_review 评审刚写的代码（内建编译器级语法检查 + 质量评审）。
      语法错误必须修复；其余问题只修 [严重]/[一般] 级，[建议] 级可不改。
      评审通过（或第 2 次评审后）即继续，不要反复评审、不要追求完美。
